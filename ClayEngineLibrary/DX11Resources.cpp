...
{
	auto hwnd = WindowSystem::GetWindowHandle();
	...
	auto backbuffer_width = UINT(WindowSystem::GetWindowWidth());
	auto backbuffer_height = UINT(WindowSystem::GetWindowHeight());
	...
	if (m_swapchain)
	{
		...
		{
			Services::GetService<RenderSystem>()->RestartRenderSystem();
