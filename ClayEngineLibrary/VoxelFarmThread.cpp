...
using SimplePerlinVoxelLayerPtr = SimplePerlinVoxelLayer*;

using MaterialLibrary = VoxelFarm::CMaterialLibrary;
using MaterialLibraryPtr = MaterialLibrary*;
using Material = VoxelFarm::CMaterial;
using Generator = VoxelFarm::CGenerator;
using GeneratorPtr = Generator*;
using CellData = VoxelFarm::CCellData;
using CellDataPtr = CellData*;
using ThreadContext = VoxelFarm::CCellData::ThreadContext;
using ThreadContextPtr = ThreadContext*;
using ContourThreadContextPtr = VoxelFarm::ContourThreadContext*;
using CellIdPtr = VoxelFarm::CellId*;

void ClayEngine::voxelfarm_thread_func::operator()(std::future<void> future)
{
	using namespace VoxelFarm;

	initPerlin(); // Static services
	LODStats stats; // Static services

	MaterialLibraryPtr materials = VF_NEW MaterialLibrary(); // Instance specific contains Materials
	...
	SimplePerlinVoxelLayerPtr layer = VF_NEW SimplePerlinVoxelLayer(); // Simple perlin noise layer implementation

	GeneratorPtr generator = VF_NEW Generator(); // Voxel generator, composes voxels from layers
	generator->addVoxelLayer(layer);

	ContourThreadContextPtr contour_context = VF_NEW ContourThreadContext(); // A thread for generating voxel contour data
	ThreadContextPtr context_cell_data = VF_NEW ThreadContext(); // A thread for generating contour mesh data

	int iterations = 0;
	int cell_count = 0;
	while ((iterations == 0 || cell_count < iterations) && future.wait_for(std::chrono::nanoseconds(1)) == std::future_status::timeout)
	{
		...
		int yc = rand();
		int zc = rand();
		CellIdPtr cell = VF_NEW CellId(packCellId(level, xc, yc, zc));
		...
		CellDataPtr cell_data = VF_NEW CellData(*cell);
		...
		std::wcout << L"LOD_" << level << L" [" << xc << L"," << yc << L"," << zc << L"] " << faces << L" faces" << std::endl;
...
ClayEngine::VoxelFarmThread::VoxelFarmThread()
{
	t = std::thread{ voxelfarm_thread_func(), std::move(p.get_future()) };
}
