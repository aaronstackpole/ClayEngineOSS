#include "pch.h"
#include "EngineCore.h"

using namespace DirectX;
using namespace ClayEngine;

using Microsoft::WRL::ComPtr;

extern void ExitGame() noexcept; // Declare extern link to Main.cpp ExitGame() function.
// We're not going to do this, instead we'll have a future from the App thread to call

#pragma region Frame Update and Draw
// Updates the world state with timing
void EngineCore::_Update(ClayEngine::Timer const& timer)
{
    auto elapsed = float(timer.GetElapsedSeconds());
    UNREFERENCED_PARAMETER(elapsed);

    //...
}

// Renders and presents the scene
void EngineCore::_Render()
{
    if (!render || render->IsDeviceLost()) return;

    // Phase 1: Clear the buffers
    render->PIXBeginEvent(L"Clear");
    auto context = render->GetContextPtr();
    auto rtv = render->GetRTVPtr();
    auto dsv = render->GetDSVPtr();
    context->ClearRenderTargetView(rtv, Colors::Black);
    context->ClearDepthStencilView(dsv, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
    context->OMSetRenderTargets(1, &rtv, dsv);
    auto viewport = render->GetViewport();
    context->RSSetViewports(1, &viewport);
    render->PIXEndEvent();

    // Phase 2: Render the scene
    render->PIXBeginEvent(L"Draw");

    //...

    render->PIXEndEvent();

    // Phase 3: Present the scene
    render->PIXSetMarker(L"Present");
    render->Present();
}

void EngineCore::Tick()
{
    //m_timer.Tick([&]() { _Update(m_timer); });
    time.Tick([&]() { _Update(time); });

    //_Draw();
}
void EngineCore::Draw()
{
    _Render();
}
#pragma endregion

#pragma region Direct3D Resources
// These are the resources that depend on the device.
void EngineCore::_Create_device_dependent_resources()
{
    //auto device = rs->GetDevicePtr();
    //auto context = rs->GetContextPtr();

    //...
}

// Allocate all memory resources that change on a window SizeChanged event.
void EngineCore::_Create_window_size_dependent_resources()
{
    //auto device = rs->GetDevicePtr();
    //auto context = rs->GetContextPtr();

    //...
}
#pragma endregion

#pragma region Constructor
EngineCore::EngineCore(WindowClassPtr wndPtr) noexcept(false)
    : window{ wndPtr }
{
    //services = std::make_unique<Services>();
    Services::AddService(L"WindowClass", reinterpret_cast<uint64_t>(wndPtr.get()));

    //render = std::make_unique<RenderSystem>(window);
    //services->AddService(L"RenderSystem", reinterpret_cast<uint64_t>(render.get()));
    render = Services::MakeService<RenderSystem>(L"RenderSystem", window);
    render->RegisterDeviceCallback(this);

    //vf = std::make_unique<VoxelFarmThread>();
    //services->AddService(L"VoxelFarmThread", reinterpret_cast<uint64_t>(vf.get()));
    vf = Services::MakeService<VoxelFarmThread>(L"VoxelFarmThread");

    //ticker = std::make_unique<TickerThread>(this);
    //services->AddService(L"TickerThread", reinterpret_cast<uint64_t>(ticker.get()));
    ticker = Services::MakeService<TickerThread>(L"TickerThread", this);

    //content = std::make_unique<ContentSystem>();
    //services->AddService(L"ContentSystem", reinterpret_cast<uint64_t>(content.get()));
    content = Services::MakeService<ContentSystem>(L"ContentSystem", render->GetDevicePtr());

    // The following two calls will be redirected to the content system
    _Create_device_dependent_resources();
    _Create_window_size_dependent_resources();

    // Change the timer settings if you want something other than the default variable timestep mode.
    // e.g. for 60 FPS fixed timestep update logic, call:
    /*
        m_timer.SetFixedTimeStep(true);
        m_timer.SetTargetElapsedSeconds(1.0 / 60);
    */
}

EngineCore::~EngineCore()
{
    content.reset();
    ticker.reset();
    vf.reset();
    render.reset();
}
#pragma endregion

#pragma region TickerThread Implementation
void ClayEngine::TickerThreadFunctor::operator()(std::future<void> future, EngineCore* core)
{
    while (future.wait_for(std::chrono::nanoseconds(0)) == std::future_status::timeout)
    {
        if (core && core->IsWindowReady())
        {
            core->Tick();
        }
    }
}

ClayEngine::TickerThread::TickerThread(EngineCore* app)
{
    t = std::thread{ TickerThreadFunctor(), std::move(p.get_future()), app };
}

ClayEngine::TickerThread::~TickerThread()
{
    p.set_value();
    if (t.joinable()) t.join();
}
#pragma endregion

#pragma region Message Handlers
void EngineCore::OnActivated()
{
    // App is becoming active window.
}

void EngineCore::OnDeactivated()
{
    // App is becoming background window.
}

void EngineCore::OnSuspending()
{
    // App is being power-suspended (or minimized).
}

void EngineCore::OnResuming()
{
    //m_timer.ResetElapsedTime();
    time.ResetElapsedTime();

    // App is being power-resumed (or returning from minimize).
}

void EngineCore::OnWindowMoved()
{
    render->WindowResized(window->GetWindowWidth(), window->GetWindowHeight());
}

void EngineCore::OnWindowResized(LONG width, LONG height)
{
    if (!render->WindowResized(width, height))
        return;

    _Create_window_size_dependent_resources();

    // App window is being resized.
}

void EngineCore::OnDeviceLost()
{
    //...
}

void EngineCore::OnDeviceRestored()
{
    _Create_device_dependent_resources();

    _Create_window_size_dependent_resources();
}
#pragma endregion
