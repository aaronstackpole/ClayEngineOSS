...
void RenderSystem::StartRenderSystem()
{
	...
	m_resources = Services::MakeService<DX11Resources>();

	// This SpriteBatch can be used for 2D games and User Interface elements
	m_spritebatch = Services::MakeService<SpriteBatch>(m_resources->GetContext());
...
void RenderSystem::StopRenderSystem()
{
	...
	if (m_spritebatch)
	{
		Services::RemoveService<SpriteBatch>();
		m_spritebatch.reset();
	...
	if (m_resources)
	{
		Services::RemoveService<DX11Resources>();
		m_resources.reset();
		m_resources = nullptr;
	}
}
...
void RenderSystem::Clear()
{
	...
	auto viewport = CD3D11_VIEWPORT{ 0.0f, 0.0f, static_cast<float>(WindowSystem::GetWindowWidth()), static_cast<float>(WindowSystem::GetWindowHeight()) };
